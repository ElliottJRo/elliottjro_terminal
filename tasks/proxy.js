...
function proxyMiddleware(req, res, next) {
  ...
  //   next();
  // }
  if(/\.(php)(\?((r|v|rel|rev)=[\-\.\w]*)?)?/.test(req.originalUrl)) {
    console.log('We have php type request! proxying to php server');
    proxy.web(req, res, { target: 'http://127.0.0.1:3010' });
  } else if(/(\/acmegw\/entitlement\/e911)(\?((r|v|rel|rev)=[\-\.\w]*)?)?/.test(req.originalUrl) ) {
    req.url = '/api.php';
    console.log('We have an API request, proxy to php server');
    proxy.web(req, res, { target: 'http://127.0.0.1:3010' });
  } else {
    next();
    //proxy.web(req, res);
